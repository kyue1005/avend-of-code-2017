...
const updateChildLevel = (node) => {
    if (!node.childs) return;

    for (let i = 0; i < node.childs.length; i++) {
        if (nodes_trees[node.childs[i]]) {
            nodes_trees[node.childs[i]].level = node.level + 1;
        } else {
            nodes_trees[node.childs[i]] = {
                level: node.level + 1,
                weight: 0,
                sum: 0,
                childs: []
        ...
        updateChildLevel(nodes_trees[node.childs[i]]);
    }
};
...
const checkBalance = (node) => {
    ...
    for (let i = 0; i < node.childs.length; i++) {
        sum += nodes_trees[node.childs[i]].sum;

        if ((sum / (i + 1)) != nodes_trees[node.childs[i]].sum) {
            is_balanced = false;
            break;
        }
    }
...
for (let i = 0; i < nodes.length; i++) {
    const label = nodes[i].split(' ')[0];
    const weight = parseInt(nodes[i].split(' ')[1].replace(/[{()}]/g, ''));

    if (nodes_trees[label]) {
        nodes_trees[label].weight = weight;
        nodes_trees[label].sum = weight;
    } else {
        nodes_trees[label] = {
            level: 0,
            weight: weight,
    ...
    if (nodes[i].split(' -> ')[1]) {
        nodes_trees[label].childs = nodes[i].split(' -> ')[1].split(', ');

        updateChildLevel(nodes_trees[label]);
    }
}

// max level
Object.keys(nodes_trees).map(l => {
    if (nodes_trees[l].level > max_level) {
        max_level = nodes_trees[l].level;
    }
});

for (let i = max_level - 1; i >= 0; i--) {
    let is_balanced = true;
    const level = Object.keys(nodes_trees).filter(l => nodes_trees[l].level === i);

    for (let i = 0; i < level.length; i++) {
        if (!checkBalance(nodes_trees[level[i]])) {
            is_balanced = false;
            console.log('Weight: ' + nodes_trees[level[i]].childs.map(l => nodes_trees[l].weight).join(', '));
            console.log('Sum   : ' + nodes_trees[level[i]].childs.map(l => nodes_trees[l].sum).join(', '));
            break;
        }
    }
    if (!is_balanced) {
