...
for (let i = 0; i < nodes.length; i++) {
    ...
    if (typeof nodes_labels[label] === 'undefined') {
        nodes_labels[label] = 0;
    }

    if (nodes[i].split(' -> ')[1]) {
        childs = nodes[i].split(' -> ')[1].split(', ');

        for (let j = 0; j < childs.length; j++) {
            if (typeof nodes_labels[childs[j]] === 'undefined') {
                nodes_labels[childs[j]] = 1;
            } else {
                nodes_labels[childs[j]]++;
            }
...
Object.keys(nodes_labels).map(l => {
    if (nodes_labels[l] === 0) {
        console.log(l);
    }
})
