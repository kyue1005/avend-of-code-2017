const input = `ioe dec 890 if qk > -10
gif inc -533 if qt <= 7
itw dec 894 if t != 0
...
const condition = (operator, reg, val) => {
    switch (operator) {
        case '>':
            return regs[reg] > val;
        case '<':
...
for (let i = 0; i < cmds.length; i++) {
    ...
    if (typeof regs[parts[0]] === 'undefined') {
        regs[parts[0]] = 0;
    }

    if (typeof regs[parts[4]] === 'undefined') {
        regs[parts[4]] = 0;
    }

    if (condition(parts[5], parts[4], parseInt(parts[6], 10))) {
        if (parts[1] === 'inc') {
            regs[parts[0]] += parseInt(parts[2], 10);
        } else if (parts[1] === 'dec') {
            regs[parts[0]] -= parseInt(parts[2], 10);
        }
    }
